  ...
  return (
    ...
    >
      <button
        ...
        type="button"
        accessKey="s"
        className={`${style.select} border border-1 dark:border-border dark:text-grey-300 border-light_gray rounded-lg w-full cursor-pointer px-4 py-3 flex justify-between items-center`}
      >
        <div className={`${style.filter} flex items-center gap-3`}>
          <Image
            ...
            height={16}
          />
          <p className="BodyM text-almost_dark dark:text-grey-300">
            {optionsSelected.name}
          </p>
        </div>
        <Image
      ...
      {optionsOpen ? (
        <div
          ...
          className={`${style.container} custom_scrollbar z-50 px-4 relative top-2 flex flex-col items-start dark:border-border dark:bg-dark-bg-1 bg-white border border-1 border-light_gray w-full rounded-lg h-96 overflow-y-scroll `}
          style={{ boxShadow: "0px 0px 32px 0px rgba(0, 0, 0, 0.10)" }}
        >
          {handleBlockOptionsAlreadySelected().map((platform, index, arr) => (
            <button
              type="button"
              role="button"
              key={index}
              ...
              className={`${platform.disabled ? style.button_disabled : ""} ${
                ...
                index !== arr.length - 1
                  ? "border-b border-1 dark:border-border border-light_gray"
                  : ""
              } BodyM text-almost_dark dark:text-grey-300 flex items-center gap-3`}
            >
              <Image
                width={16}
                height={16}
